...
def test_adoption_is_first_writer_wins(profile_homes):
    ...
    # The rejected handle must have been CLOSED, not orphaned. Probe the
    # connection instead of counting descriptors here: since the 0.20 runtime
    # one open SessionDB holds a variable number of fds (write handle,
    # per-thread read-only pool — upstream 67d4bbb81 — plus WAL side files
    # whose lifetime shifts with sibling connections), so an absolute
    # intermediate fd assertion is noise. The baseline assertion below is the
    # leak check.
    assert second._conn is None  # SessionDB.close() drops the write handle
...
def _server_ast() -> ast.Module:
    """Union AST of server.py and the methods_* split modules.

    The 0.20 handler split (f67ca220a) moved ``session.resume`` /
    ``session.branch`` — and with them the ``_session_resume`` mint site —
    into ``tui_gateway/methods_session.py``; the handlers are rebound onto
    server.py's globals at install time, so structurally they are one module.
    """
    import tui_gateway.methods_session as _ms

    tree = ast.parse(Path(srv.__file__).read_text(encoding="utf-8"))
    tree.body.extend(
        ast.parse(Path(_ms.__file__).read_text(encoding="utf-8")).body
    )
    return tree
