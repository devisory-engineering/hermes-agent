...
def _adopt_owned_session_db(session: dict | None, db) -> None:
    ...
    keeps the first one, so a rebuild can never orphan the original.
    """
    if not isinstance(session, dict) or db is None:
        return
    if session.get(_OWNED_DB_KEY) is None:
